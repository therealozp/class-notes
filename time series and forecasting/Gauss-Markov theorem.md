...
where $\sigma_{i} \neq \sigma_{j}$.
## serial correlation
if there is [[serial correlation]], then all the terms inside the std dev matrix are no longer zero. instead: 
$$\begin{bmatrix}
...
if **any** of these $\sigma_{ij} \neq 0$, we have a serial correlation. the [[serial correlation|Durbin-Wattson test]] checks if $\sigma_{i, i-1}$ is non-zero (does not depend on previous term).
