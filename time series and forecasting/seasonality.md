...
typically, $\text{uncentered }R^{2}\geq \text{centered } R^{2}$. note that if the sample mean $\bar{y}=0$, centered and uncentered $R^2$ are equal. 

>[!NOTE] note
>we only use **centered** $R^2$, never uncentered.
